...
type ReferrerSourceResult struct {
	ReferrerSource       string
	PercentageOfReferrer float64
}

// TODO: Migrate to repository layer
func TopReferrersHandler(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		...
		SELECT 
    		...
    		) as PercentageOfReferrer
		FROM roving.web_traffic_event, TotalSessions
		WHERE SiteId = ? AND toTimeZone(Timestamp, ?) BETWEEN ? AND ?
		GROUP BY ReferrerSource, TotalSessionsCount
		ORDER BY PercentageOfReferrer DESC
	`
	...
	var results []ReferrerSourceResult = make([]ReferrerSourceResult, 0)

	for rows.Next() {
		var result ReferrerSourceResult
		if err := rows.Scan(&result.ReferrerSource, &result.PercentageOfReferrer); err != nil {
			log.Default().Println(err)
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}
